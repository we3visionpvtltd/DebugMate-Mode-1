...
import { FiArrowUp, FiEdit3, FiSave, FiX } from 'react-icons/fi';
...
const ProjectDetailsTable = () => {
  ...
  // Permission hooks
  const { canView, canUpdate, canDelete, hasPermission } = usePermissions();
  ...
  <div className="projects-container-custom ">
    {projects.length === 0 ? (
      ...
      projects.map((proj, idx) => (
        <Card key={proj.uuid || proj.id || idx} className="mb-3">
          <Card.Body>
            ...
            <div className="project-details small text-muted">
              ...
              <div><strong>Client:</strong> {proj.client_name || proj.clientName || "Not Set"}</div>
