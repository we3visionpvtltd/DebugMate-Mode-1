...
const WorkChat = () => {
  ...
  // ✅ Storage keys - now using project-specific keys for both individual and unified storage
  const storageKey = `chatHistory_project_${projectId}`;
  const unifiedStorageKey = userEmail 
    ? `unified_chatHistory_${userEmail}_${projectId}` 
    : `unified_chatHistory_guest_${projectId}`;
  ...
  useEffect(() => {
    if (chatHistory.length > 0) {
      try {
        localStorage.setItem(storageKey, JSON.stringify(chatHistory));
        // Also save to project-specific unified storage
        const projectChats = chatHistory.filter(chat => chat?.projectId === projectId);
        if (projectChats.length > 0) {
          localStorage.setItem(unifiedStorageKey, JSON.stringify(projectChats));
        }
      } catch (error) {
