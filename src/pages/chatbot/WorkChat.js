...
const WorkChat = () => {
  ...
  useEffect(() => {
    ...
    const fetchProject = async () => {
      if (!userEmail) return;
      try {
        const projectResponse = await fetch("https://zeelsheta-debugmate_backend.hf.space/get_user_project", {
          method: "POST",
          headers: { "Content-Type": "application/json" ,"Authorization": "Bearer debugmate123" },
          credentials: "include",
          body: JSON.stringify({ email: userEmail })
...
const sendMessage = async () => {
  ...
  try {
    const response = await fetch("https://zeelsheta-debugmate_backend.hf.space/chat/work", {
      method: "POST",
      headers: { "Content-Type": "application/json" ,
        "Authorization": "Bearer debugmate123" },
      credentials: "include",
