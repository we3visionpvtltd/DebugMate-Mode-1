...
if (typeof window !== 'undefined') {
    try {
        // Load main counters
        const savedCounters = localStorage.getItem('debugmate_counters');
        if (savedCounters) {
        ...
        const savedProjCounters = localStorage.getItem('debugmate_project_id_counters');
        ...
        // Load chat counters
        const savedChatCounters = localStorage.getItem('debugmate_chat_counters');
...
const saveCounters = () => {
    if (typeof window !== 'undefined') {
        try {
            localStorage.setItem('debugmate_counters', JSON.stringify({
                company: companyCounter,
                project: projectCounter
            }));
            localStorage.setItem('debugmate_project_id_counters', JSON.stringify({
...
const getChatCounter = (projectId = 'default') => {
    if (!chatCounters.has(projectId)) {
        ...
        if (typeof window !== 'undefined') {
            try {
                const chatCountersData = JSON.parse(localStorage.getItem('debugmate_chat_counters') || '{}');
